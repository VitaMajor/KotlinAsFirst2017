...
data class Circle(val center: Point, val radius: Double) {
    ...
    fun distance(other: Circle): Double {
        val op = Math.sqrt(sqr(center.x - other.center.x) +
                sqr(center.y - other.center.y))- (radius + other.radius)
        if (op > 0) {
            return op
        }
            return 0.0
...
fun circleByDiameter(diameter: Segment): Circle = Circle(Point((diameter.begin.x + diameter.end.x) / 2,
        ...
        Math.sqrt(sqr(diameter.begin.x - diameter.end.x) + sqr(diameter.begin.y - diameter.end.y)) / 2)
